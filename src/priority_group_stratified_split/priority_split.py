...
class PrioritySplit(Split):
    def _backward(self, sums_dict:dict[int:int], closest_sum:int, group_set:GroupSet) -> GroupSet:
        """
        Args:
            sums_dict: a dict of type {POSSIBLE_SUM: GROUP_UID}
    ...
    def _best_group_set(self, group_set: GroupSet, ideal_size) -> GroupSet:
        ...
        """
        # create a dict in the form sum_value:group_uid
    ...
    def _stratified_set(self, group_set:GroupSet, p) -> GroupSet:
        ...
        Args:
            ...
            p: proportion of number of samples in this split set over all samples

        Returns:
            A set of groups splited by classes in a stratified maner with samples amounting
            to a value as close as possible to `group_set.total_size` * `p`
        """
        new_set = GroupSet()
    ...
    def get_split(self, group_set:GroupSet, percentages:list[float], tolerance:float=0.05) -> list[GroupSet]:
        ...
        Args:
            ...
            tolerance: how much the sum of percentages can deviate from 1

        Returns:
            A list of GroupSets with size equal to `percentages`.
        ...
        sizes[-1] = total_size - sum(sizes[:-1]) # this is actually never used, but sizes[-1] would be storing a wrong value
